...
public class Driver_prj0 {
  ...
  public static void main(String[] args) {
    ...
    // error when stack is empty and a function is returned
    int error_empty_stack = 1;
    // error when the returned function doesn't match top of stack
    int error_stack_mismatch = 2;
    ...
    while (input.hasNext()) {
      ...
      if ( line.indexOf("call") != -1) {
        ...
        if (callStack.isEmpty()) {
          ...
          // holds what is currently in the top of stack
    	    topOfStack = callStack.peek();
    	    if (!topOfStack.equals(function)) {
    	      // if the function on top of the stack != the current function
    	      errorCode = error_stack_mismatch;
    	      break;
    	    } else if (topOfStack.equals(function)) {
    	      // if the function in the top of the stack == the current function
    	      callStack.pop();
    	      current_depth--;
    	    }
    	  }
    ...
    if (callStack.isEmpty() && errorCode == 0) {
      ...
      if (errorCode == 1){
        ...
        // prints out the stack trace but if this error is called,
        // there shouldn't be anything in stack
